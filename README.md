...
- **シールライブラリ** — 切り抜いたシールをコレクションとして一覧管理、新着順/古い順の並び替え対応、長押しメニューから不要部分の再除去・向きの回転（左/右90度）が可能。ナビバー「選択」ボタンで複数選択モードに切り替えて一括削除できる
- **カルーセル型ボード一覧** — ホーム画面でボードをカルーセル形式で大きく表示、シール配置のプレビュー付き（初回起動時にデフォルトボードを自動作成）
- **タブナビゲーション** — フローティングタブバーでホーム・撮影・ライブラリを切り替え
- **ボードエディタ** — シールをボード上にドラッグ・ピンチ・回転で自由に配置、タップで選択してレイヤー操作・削除。ナビゲーションバーの「元に戻す」ボタンで最大20ステップの操作を取り消し可能
- **キャンバスズームモード** — ツールバーの「ズーム」ボタンでキャンバス全体を拡縮（0.3x〜5.0x）・スクロール可能なズームモードをON/OFF切替。小画面端末でシールが密集したボードを操作しやすい
- **シールロック** — ボード上のシールを個別にロックして位置・サイズ・回転を固定。誤操作によるずれを防止。ロック中は効果・枠線・削除操作も無効化
...
| ライブラリ | 保存したシールがグリッドで表示される・ナビバーの並び替えボタンで新着順/古い順を切り替え可能・先頭の「さらに追加」カードからシール追加可能・タップでプレビュー拡大・長押しで「不要部分を除去」（マスク再編集）・左/右に90度回転・削除ができる・ナビバー「選択」→ チェックして「N枚を削除」で複数まとめて削除 |
| ボード作成 | 新規ボードを作成（通常 or ウィジェット用を選択） → ボード一覧に表示される |
...
| シール選択・操作 | シールをタップで選択 → 下部ツールバーの「前面へ」「背面へ」「削除」で操作。「ロック」ボタンでシールを固定（ロック中はドラッグ・ピンチ・回転・削除が無効化される） |
| ボード画像保存 | ボード編集画面のダウンロードボタン → 写真ライブラリに画像として保存される |
| データ永続化 | アプリを終了→再起動してシール・ボードが残っている |
| ウィジェット | ホーム画面にウィジェットを追加（Small / Medium / Large）→ ボードを選択 → スナップショット表示 → タップでボード編集画面に遷移 |
