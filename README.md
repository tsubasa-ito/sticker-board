...
- **シールライブラリ** — 切り抜いたシールをコレクションとして一覧管理、長押しメニューから不要部分の再除去が可能
...
- **iCloudバックアップ（Pro限定）** — シール・ボードのデータをiCloudにバックアップ。SwiftData CloudKit連携によるモデルデータの自動同期＋画像ファイルのiCloud Drive双方向同期。設定画面からバックアップ状態の確認・手動同期が可能。機種変更時のデータ移行をサポート
- **Pro サブスクリプション** — フリーミアムモデル（無料: シール30枚・ボード1枚・基本枠線・基本背景 / Pro: 無制限・全機能・ロゴなし書き出し）。StoreKit 2 による月額¥380 / 年額¥2,900
- **設定画面** — 現在のプラン表示・有効期限確認・月額/年額プラン選択＆直接購入・プラン管理（Pro→App Storeサブスクリプション管理）・購入復元・iCloudバックアップ状態表示＆手動同期（Pro限定）・Proメリット一覧・よくある質問・注意事項・関連リンク
...
| データ保存 | SwiftData + FileManager + CloudKit（iCloud同期） |
...
| ライブラリ | 保存したシールが最新順のグリッドで表示される・先頭の「さらに追加」カードからシール追加可能・タップでプレビュー拡大・長押しで「不要部分を除去」（マスク再編集）・削除ができる |
...
| iCloudバックアップ | 設定画面「iCloudバックアップ」セクション → Proユーザーで同期ステータス表示・「今すぐバックアップ」ボタン → 無料ユーザーはPro限定表示 |
...
│   ├── ICloudSyncManager.swift      # iCloud同期マネージャ（Pro判定・ステータス管理）
│   └── ImageSyncService.swift       # 画像ファイルのiCloud Drive双方向同期
└── Views/
    ├── Home/
...
StickerBoardTests/                           # Swift Testing ユニットテスト
    ...
    ├── ImageStorageTests.swift              # PNG保存・読み込み・削除サイクル
    ├── BackgroundImageStorageTests.swift    # JPEG保存・読み込み・削除サイクル
    ├── UIImageExtensionTests.swift          # リサイズ・アルファトリミング
    ...
    ├── ICloudSyncManagerTests.swift                 # iCloud同期マネージャのステータス管理・Pro判定
    └── ImageSyncServiceTests.swift                  # 画像ファイル同期の双方向コピー・スキップ
```
