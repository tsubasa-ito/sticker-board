...
- **シールライブラリ** — 切り抜いたシールをコレクションとして一覧管理、新着順/古い順の並び替え対応、長押しメニューから不要部分の再除去・向きの回転（左/右90度）が可能
...
| ライブラリ | 保存したシールがグリッドで表示される・ナビバーの並び替えボタンで新着順/古い順を切り替え可能・先頭の「さらに追加」カードからシール追加可能・タップでプレビュー拡大・長押しで「不要部分を除去」（マスク再編集）・左/右に90度回転・削除ができる |
