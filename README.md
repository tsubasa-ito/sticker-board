...
- **シール切り抜き** — カメラロールから写真を選び、Vision Framework の AI が背景を自動除去
- **シールライブラリ** — 切り抜いたシールをコレクションとして一覧管理
- **ボードエディタ** — シールをボード上にドラッグ・ピンチ・回転で自由に配置
...
| 機能 | 確認内容 |
|------|---------|
| シール追加 | 写真を選択 → 背景除去 → プレビュー表示 → ライブラリに保存 |
| ライブラリ | 保存したシールがグリッドで表示される・長押しで削除できる |
