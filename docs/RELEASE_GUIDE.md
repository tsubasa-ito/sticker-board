...
mainブランチへのマージ（develop → main）をトリガーに、自動で TestFlight 配信や App Store 提出を行えます。日常の開発は develop ブランチに統合し、リリース時に main へマージします。
...
```
develop → main にマージ → git push (main) → Xcode Cloud がトリガー
