...
struct StickerBorderServiceTests {
    ...
    func 枠線適用後にnilでない画像が返る(width: StickerBorderWidth) {
        let image = makeAlphaTestImage()
        let result = StickerBorderService.applyBorder(to: image, width: width, colorHex: "FF0000")
    ...
    @Test func 枠線適用後のピクセルサイズが元画像と同じ() {
        let image = makeAlphaTestImage(size: CGSize(width: 300, height: 250))
        let result = StickerBorderService.applyBorder(to: image, width: .medium, colorHex: "000000")
    ...
    func 各カラーhexで枠線適用がクラッシュしない(hex: String) {
        let image = makeAlphaTestImage()
        let result = StickerBorderService.applyBorder(to: image, width: .medium, colorHex: hex)
